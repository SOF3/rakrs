...
use syn::{parse_macro_input, Attribute, Data, DeriveInput, Error, Fields, Ident, Result, Type};
...
fn imp(item: DeriveInput) -> Result<TokenStream> {
...
fn write_field(_ty: &Type, expr: &TokenStream) -> Result<TokenStream> {
    Ok(quote! {
        crate::protocol::CanIo::write(&(#expr), w)?;
    })
}
...
fn read_field(ty: &Type) -> Result<TokenStream> {
    Ok(quote! {
        {
            let var: #ty = crate::protocol::CanIo::read(r)?;
            var
        }
    })
}
